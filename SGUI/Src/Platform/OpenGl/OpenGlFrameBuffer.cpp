...
namespace SGUI {
	...
	namespace Utils {
		...
		static unsigned int TextureTarget(bool multisampled)
		{
		...
		static void AttachColorTexture(uint32_t id, int samples,unsigned int internalformat, unsigned int format, uint32_t width, uint32_t height, int index)
		{
		...
		static void AttachDepthTexture(uint32_t id, int samples, unsigned int format, unsigned int AttachmentType, uint32_t width, uint32_t height)
		{
			bool multisampled = samples > 1;
			if (multisampled)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			SG_CORE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	{
		SG_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
