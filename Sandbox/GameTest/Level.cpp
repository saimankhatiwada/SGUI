...
void Level::Init()
{
	m_TriangleTexture = SGUI::Texture2D::Create("Assets/Image/Triangle.png");
	m_Player.LoadAssets();
...
void Level::OnUpdate(SGUI::TimeSteps ts)
{
...
void Level::OnRender()
{
	...
	// Background
	SGUI::Renderer2D::DrawQuad({ playerPos.x, 0.0f, -0.8f }, { 50.0f, 50.0f }, { 0.3f, 0.3f, 0.3f, 1.0f });

	// Floor and ceiling
	SGUI::Renderer2D::DrawQuad({ playerPos.x,  34.0f }, { 50.0f, 50.0f }, color);
	SGUI::Renderer2D::DrawQuad({ playerPos.x, -34.0f }, { 50.0f, 50.0f }, color);

	for (auto& pillar : m_Pillars)
	{
		SGUI::Renderer2D::DrawRotatedQuad(pillar.TopPosition, pillar.TopScale, 180.0f, m_TriangleTexture, 1.0f, color);
		SGUI::Renderer2D::DrawRotatedQuad(pillar.BottomPosition, pillar.BottomScale, 0.0f, m_TriangleTexture, 1.0f, color);
	}
